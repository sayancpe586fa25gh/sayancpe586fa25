...
from wgan import MaskedWGANGenerator, MaskedWGANCritic, MaskedWGANTrainer
    ...
    for each_file in all_files:
        ...
        for row in range(rows):
            for col in range(0, cols - segment_size + 1, segment_size):
                ...
                # -------------------------------
                # ✅ MASKED WGAN MODEL & TRAINER
                # -------------------------------
                generator = MaskedWGANGenerator(input_dim=2*segment_size, output_dim=segment_size).to(device)
                critic = MaskedWGANCritic(input_dim=2*segment_size).to(device)
                ...
                trainer = MaskedWGANTrainer(
                    input_dim=2*segment_size,
                    output_dim=segment_size,
                    generator=generator,
                    critic=critic,
                ...
                for epoch in range(15):
                    ...
                    print(f"Epoch {epoch+1} | G: {g_loss:.4f} | D: {d_loss:.4f} | Val Score: {val_score:.4f}")


                # ---- test (HD + accuracy using masking) ----
                ...
                with torch.no_grad():
                    for i, x in enumerate(test_loader):
                        ...
                        # ✅ apply SAME masking logic as used in training
                        # Generator produces synthetic segments
                        x_masked, mask = trainer.mask_segment(x)
                        x_hat = trainer.sample(x_masked, mask)
                        #print(x,x_hat)
                        real = (x > 0.5)
                        pred = (x_hat > 0.5)
                        missing = (mask == 0)
