...
from sayancpe586fa25.genAI.vae import VAE, VAETrainer
    ...
    for each_file in all_files:
        ...
        for row in range(rows):
            for col in range(0, cols - segment_size + 1, segment_size):
                ...
                # ---- NEW MODEL PER SEGMENT ----
                model = VAE(input_dim=segment_size, latent_dim=64).to(device)
                optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)
                trainer = VAETrainer(model=model, optimizer=optimizer, device=device)

                # ---- datasets (cycle split) ----
                train_ds = SRAMSegmentPerIndexDataset(each_file, row, col, split="train", segment_size=segment_size, multi_cycle=True)
                val_ds   = SRAMSegmentPerIndexDataset(each_file, row, col, split="val", segment_size=segment_size)
                test_ds  = SRAMSegmentPerIndexDataset(each_file, row, col, split="test", segment_size=segment_size)
                ...
                with torch.no_grad():
                    for i, x in enumerate(test_loader):
                        ...
                        print(f"Batch {i} HD: {hd_pct:.2f}")

                        if i >= 10:
                            break

                # 🔴 IMPORTANT: remove break later
            #break
        break
